...
def _list_keys(client) -> list[str]:
    resp = client.list_objects_v2(Bucket=BUCKET, Prefix="dt-cz/")
    return sorted(o["Key"] for o in resp.get("Contents", []))


def test_writes_latest_and_timestamped_snapshot(s3_bucket, env):
    env.update({"BUCKET": BUCKET, "PREFIX": "dt-cz", "REGION": REGION})
    docs = _docs()

    out = S3MarkdownStep().run(docs)

    keys = _list_keys(s3_bucket)
    assert "dt-cz/latest.json" in keys
    ts_keys = [k for k in keys if k != "dt-cz/latest.json"]
    assert len(ts_keys) == 1, f"expected one timestamped snapshot, got {ts_keys}"

    expected = [d.model_dump() for d in docs]
    latest_body = json.loads(s3_bucket.get_object(Bucket=BUCKET, Key="dt-cz/latest.json")["Body"].read())
...
def test_metadata_field_preserved(s3_bucket, env):
    ...
    body = json.loads(s3_bucket.get_object(Bucket=BUCKET, Key="dt-cz/latest.json")["Body"].read())
    assert body[0]["metadata"] == {"char_len": 6}
    assert body[1]["metadata"] is None


def test_provenance_object_metadata(s3_bucket, env):
    env.update({"BUCKET": BUCKET, "PREFIX": "dt-cz", "REGION": REGION, "TENANT": "cz"})
    S3MarkdownStep().run(_docs())
    head = s3_bucket.head_object(Bucket=BUCKET, Key="dt-cz/latest.json")
...
def test_put_error_raises_stepfailed(aws_creds, env):
    ...
    with mock_aws():
        env.update({"BUCKET": "does-not-exist", "PREFIX": "dt-cz", "REGION": REGION})
